	...
	<div class="w-full max-w-3xl mx-auto">
		...
		<div class="mb-6 text-center">
			...
			<div class="mt-2 h-2 bg-dungeon-medium rounded-full overflow-hidden">
				<div class="h-full bg-gradient-to-r from-gold to-gold-light transition-all duration-500" :style="{ width: `${(questionNumber / totalQuestions) * 100}%` }"></div>
			</div>
		</div>

		<!-- Question -->
		<div class="bg-dungeon-medium border-2 border-gold/30 rounded-lg p-6 mb-6 shadow-lg">
			<h2 class="text-xl md:text-2xl text-parchment font-semibold leading-relaxed" v-html="question.question"></h2>
		</div>

		<!-- Hint Button -->
		<div class="mb-6 text-center" v-if="!showHint">
			<button @click="showHint = true" class="text-gold/70 hover:text-gold text-sm underline underline-offset-4 transition-colors">Afficher l'indice</button>
		</div>

		<!-- Hint -->
		<div v-if="showHint" class="mb-6 bg-gold/10 border border-gold/30 rounded-lg p-4 text-gold-light text-sm italic"><span>💡 </span><span v-html="question.hint"></span></div>
		...
		<div class="space-y-3 mb-6">
			<button
				...
				:class="['w-full text-left p-4 rounded-lg border-2 transition-all duration-300', getOptionClass(option), selectedAnswer === option && !hasAnswered ? 'border-gold ring-2 ring-gold/50' : 'border-transparent']">
				<div class="flex items-start gap-3">
					...
					<span class="text-parchment" v-html="option.text"></span>
				</div>
			</button>
		</div>

		<!-- Rationale -->
		<div v-if="hasAnswered && selectedAnswer" class="mb-6 p-4 rounded-lg border-2" :class="selectedAnswer.isCorrect ? 'bg-emerald/10 border-emerald/50' : 'bg-blood/10 border-blood/50'">
			...
			<p class="text-parchment/80 text-sm" v-html="selectedAnswer.rationale"></p>
		</div>
		...
		<div class="text-center">
			...
			<button v-else @click="$emit('answer', selectedAnswer!.isCorrect)" class="px-8 py-3 bg-gradient-to-r from-gold to-gold-light text-dungeon-dark font-bold rounded-lg transition-all duration-300 hover:shadow-lg hover:shadow-gold/30">
				Question suivante →
			</button>
		</div>
	</div>
