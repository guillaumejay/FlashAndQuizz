...
<template>
  <div class="w-full max-w-3xl mx-auto">
    ...
    <!-- Flashcard -->
    <div
      class="perspective-1000 mb-6 cursor-pointer"
      @click="flipCard"
    >
      <div
        class="relative w-full min-h-[300px] transition-transform duration-500 transform-style-preserve-3d"
        :class="{ 'rotate-y-180': isFlipped }"
      >
        <!-- Front -->
        <div
          class="absolute inset-0 backface-hidden bg-dungeon-medium border-2 border-gold/30 rounded-xl p-8 shadow-xl flex flex-col justify-center"
        >
        ...
        <!-- Back -->
        <div
          class="absolute inset-0 backface-hidden rotate-y-180 bg-dungeon-light border-2 border-gold/50 rounded-xl p-8 shadow-xl flex flex-col justify-center"
        >
          <div class="text-center mb-4">
    ...
    <div v-if="isFlipped" class="flex justify-center gap-4">
      <button
        @click="markAsUnknown"
        class="px-6 py-3 bg-blood/20 border-2 border-blood text-red-300 font-bold rounded-lg transition-all duration-300 hover:bg-blood/40"
      >
        ✗ Je ne savais pas
      </button>
      <button
        @click="markAsKnown"
        class="px-6 py-3 bg-emerald/20 border-2 border-emerald text-emerald font-bold rounded-lg transition-all duration-300 hover:bg-emerald/40"
      >
        ✓ Je savais
      </button>
    </div>
    ...
    <div v-else class="text-center">
      <p class="text-parchment/50 text-sm">Cliquez sur la carte pour voir la réponse</p>
    </div>
    ...
    <div class="mt-8 text-center">
      <button
        @click="$emit('quit')"
        class="text-parchment/40 hover:text-parchment/70 text-sm underline underline-offset-4 transition-colors"
      >
